...
既存Issueに紐付けてブランチを作成して作業した場合、ユーザーからPR作成を依頼されたタイミングでPRを作成し、その本文に `Closes #<Issue番号>` を含める。これによりGitHub上のPRのDevelopment欄とIssueを紐付け、マージ時にIssueを自動でクローズする。

PR作成後は、本文と紐付いたIssueを確認する。PRには変更内容と実行した確認コマンドを簡潔に記載する。
